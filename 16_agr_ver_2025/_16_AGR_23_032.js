...
webform.validators.agr16 = function (v, allowOverpass) {
    ...
    // Define the rows for validation
    var validationRows = ["0100", "0200", "0300", "0400"];

    for (var i = 0; i < validationRows.length; i++) {
        var row = validationRows[i];

        // Initialize variables to 0 before assigning values
        var col1 = 0, col3 = 0, col2 = 0, col4 = 0, col5 = 0, col7 = 0, col6 = 0, col8 = 0;

        col1 = !isNaN(Number(values["CAP22_R" + row + "_C1"])) ? Number(values["CAP22_R" + row + "_C1"]) : 0;
        col3 = !isNaN(Number(values["CAP22_R" + row + "_C3"])) ? Number(values["CAP22_R" + row + "_C3"]) : 0;

        if ((col1 > 0 && col3 == 0) || (col3 > 0 && col1 == 0)) {
            webform.errors.push({
                'fieldName': 'CAP22_R' + row + '_C1',
                'weight': 8,
                'msg': Drupal.t('Cod eroare: 35-008. Daca Coloana 1 (pentru rindurile 0100, 0200, 0300, 0400) <> 0, atunci Coloana 3 <> 0 si invers - [@col1, @col3]', { '@col1': col1, '@col3': col3 })
            });
        }

        col2 = !isNaN(Number(values["CAP22_R" + row + "_C2"])) ? Number(values["CAP22_R" + row + "_C2"]) : 0;
        col4 = !isNaN(Number(values["CAP22_R" + row + "_C4"])) ? Number(values["CAP22_R" + row + "_C4"]) : 0;

        if ((col2 > 0 && col4 == 0) || (col4 > 0 && col2 == 0)) {
            webform.errors.push({
                'fieldName': 'CAP22_R' + row + '_C2',
                'weight': 9,
                'msg': Drupal.t('Cod eroare: 35-009. Daca Coloana 2 (pentru rindurile 0100, 0200, 0300, 0400) <> 0, atunci Coloana 4 <> 0 si invers - [@col2, @col4]', { '@col2': col2, '@col4': col4 })
            });
        }

        col5 = !isNaN(Number(values["CAP22_R" + row + "_C5"])) ? Number(values["CAP22_R" + row + "_C5"]) : 0;
        col7 = !isNaN(Number(values["CAP22_R" + row + "_C7"])) ? Number(values["CAP22_R" + row + "_C7"]) : 0;

        if ((col5 > 0 && col7 == 0) || (col7 > 0 && col5 == 0)) {
            webform.errors.push({
                'fieldName': 'CAP22_R' + row + '_C5',
                'weight': 10,
                'msg': Drupal.t('Cod eroare: 35-010. Daca Coloana 5 (pentru rindurile 0100, 0200, 0300, 0400) <> 0, atunci Coloana 7 <> 0 si invers - [@col5, @col7]', { '@col5': col5, '@col7': col7 })
            });
        }

        col6 = !isNaN(Number(values["CAP22_R" + row + "_C6"])) ? Number(values["CAP22_R" + row + "_C6"]) : 0;
        col8 = !isNaN(Number(values["CAP22_R" + row + "_C8"])) ? Number(values["CAP22_R" + row + "_C8"]) : 0;

        if ((col6 > 0 && col8 == 0) || (col8 > 0 && col6 == 0)) {
            webform.errors.push({
                'fieldName': 'CAP22_R' + row + '_C6',
                'weight': 11,
                'msg': Drupal.t('Cod eroare: 35-011. Daca Coloana 6 (pentru rindurile 0100, 0200, 0300, 0400) <> 0, atunci Coloana 8 <> 0 si invers - [@col6, @col8]', { '@col6': col6, '@col8': col8 })
            });
        }
    }
    ...
    for (var i = 10; i <= 270; i++) {
        {
            if (i < 85 && fun_row_36_001(i)) {
                if (
                    ...
                    var SUM_36_002  = 0;
                    SUM_36_002  = col7 + col8 + col9 + col10 + col14 + col15;
